...
# PBX connected?
grep "Connected successfully" /home/pi/Hotel-ECS/logs/pbx_listener.log
...
└─ Dashboard not loading?
   ...
   ├─ Fix: Copy dashboard to backend/public/
...
### Backup Database
```bash
cp /home/pi/Hotel-ECS/snc-poc/backend/nurse_call_events.db \
...
tar czf /home/pi/backups/logs_$(date +%Y%m%d).tar.gz \
   /home/pi/Hotel-ECS/logs/
```

### Restore Database
```bash
cp /path/to/backup.db /home/pi/Hotel-ECS/snc-poc/backend/nurse_call_events.db
./start-snc-system.sh
```
