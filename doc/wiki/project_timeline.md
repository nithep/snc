...
## [2026-09-07] Deploy ADR 0014 (Remove Kiosk Scaling) ขึ้น Production + Visual/Test/Sync Verification

**ผู้ดำเนินการ:** Senior Software Engineer (Buffy — Freebuff Agent)

**รายละเอียด:**
- **นำ [ADR 0014](doc/adr/0014-remove-kiosk-scaling.md) ขึ้นระบบจริง (commit `cbebdac`)** ผ่าน `ops/deploy-snc-one-shot.sh --check-tunnel` ไปยัง Pi4 (`hotel-gateway` / `192.168.1.94`):
  - `app/index.html`: ลบ Kiosk Scaling (`#appScale`, `fitToScreen()`, `ResizeObserver`) ออกจาก dashboard — กลับสู่ responsive natural flow (`overflow-y: auto` + scroll แนวตั้ง) ตาม ADR 0014
...
- **สถานะ:** ✅ Live dashboard = local = GitHub (ชุด runtime ครบ) — ADR 0014 มีผลเต็มรูปแบบบน snc.nithep.com
...
## [2026-09-11] API Key Rotation + .gitignore Hardening + Pi4 Sync + ลบ Cloud Run
...
- **ลบ Cloud Run (`snc-cloud-backend`):** ตัดสินใจลบเพราะ: (1) Pi4 ทำหน้าที่ครบแล้ว (2) billing ปิดอยู่ (3) เคยมี data loss (4) ไม่มี traffic จริง → ลบสำเร็จ ✅
...
**�����Թ���:** Senior Software Engineer (opencode)

**��������´:**
- **Git:** HEAD 67edf98 = origin/main, working tree ���Ҵ � ����� commit �������� 12/9
- **Tests:** pytest tests/ pbx/test_smdr_parser.py  **76 passed + 4 subtests**
- **Arch:** �׹�ѹ Pi4-only (SQLite WAL, ����� Cloud Run/GCP) � �������¹�ҡ 09-12
- **�͡���:** [[SESSION_HANDOVER_2026-09-18]]
- **��ҧ:** live verify �� Pi4 (health + synthetic trigger), E2E field test, �ѻവ SKILL.md STEP 12 latest-handover, ��Ǩ cron backup DB

- **ʶҹ�:** ? Steady state � repo ���Ҵ + test ��ҹ�ú
