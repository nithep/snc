...
BACKUP_DIR="/home/ecs-agent/snc/backups"
RETENTION_DAYS=14
...
if [ "$INSTALL_MODE" -eq 1 ]; then
  # ติดตั้ง cron บน Pi
  CRON_LINE="0 3 * * * /home/ecs-agent/snc/ops/backup-snc-db.sh --pi >/dev/null 2>&1"
  if [ "$PI_MODE" -eq 1 ]; then
    ...
    scp -o ConnectTimeout=8 "$0" pi4:/home/ecs-agent/snc/ops/backup-snc-db.sh
    ssh -o ConnectTimeout=8 pi4 "chmod +x /home/ecs-agent/snc/ops/backup-snc-db.sh"
    # รันติดตั้ง cron ด้วยตัวเองบน Pi
    ssh -o ConnectTimeout=8 pi4 "/home/ecs-agent/snc/ops/backup-snc-db.sh --install --pi"
  fi
  exit 0
fi
