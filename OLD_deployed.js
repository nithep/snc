...
function renderStatusStrip() {
  ...
  if (!el) return;
  const vals = Object.values(roomStates);
...
function updateBanner() {
  const banner = document.getElementById('emergencyBanner');  const ems = Object.entries(roomStates).filter(([, s]) => s.status === 'emergency')
